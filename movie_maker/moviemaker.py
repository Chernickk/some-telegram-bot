...
class MovieMaker:
    ...
    def get_video(self, start_timeframe, finish_timeframe, file_name):
        ...
        self.logger.info(f"Время начала записи: {timeframe['start']}, время конца записи: {timeframe['finish']}. "
                         f"Камера {self.camera['cameraId']} {self.camera['cameraURL']}. Длина {timeframe['duration']} сек")
    ...
    def get_video_from_server(self, timeframe, camera, result_file_name):
        ...
        try:
            frame_list = requests.post(self.camera["cameraURL"], auth=HTTPDigestAuth(login, password),
            ...
            if len(frames_id) > 0:
                ...
                request_args = [{"method": "archive.get_frame",
                                 "params": {"channel": camera['cameraId'], "stream": "video",
                                            "id": frame}} for frame_keys in frames_id for frame in frame_keys]
                ...
                with open(result_file_name, 'wb') as result_file:
                    [result_file.write(frame[b'result'][b'frame'][b'raw_bytes']) for frame in frames]
